 ...
 */ function handleCustomCommands(command) {
    switch(command){
        ...
        case "light":
            if (document.body.classList.length === 0) return "Vous êtes déjà en mode clair";
            _customComands.setDarkMode(false);
            return "Vous êtes maintenant en mode clair.";
        case "dark":
            if (document.body.classList.length === 1) return "Vous êtes déjà en mode sombre";
            _customComands.setDarkMode(true);
            return "Vous êtes maintenant en mode sombre.";
        case "get cv dev":
            _customComands.getCVDev();
            return "Le CV va être téléchargé.";
        case "get cv sec":
            _customComands.getCVSec();
            return "Le CV va être téléchargé.";
        case "rm -rf /":
            _customComands.rmRf();
...
parcelHelpers.export(exports, "getCVDev", ()=>getCVDev
);
parcelHelpers.export(exports, "getCVSec", ()=>getCVSec
);
